...
void CatalogManager::createTable(std::string name, Attribute Attr, int primary, Index index){
    ...
    for(int current_block=0;current_block<block_num;current_block++){
        char* buffer = BM.getPage(TABLE_MANAGER_PATH , current_block);
        int page_id = BM.getPageId(TABLE_MANAGER_PATH , current_block);
        //寻找该block的有效长度
        int length=0;
    ...
    char* buffer = BM.getPage(TABLE_MANAGER_PATH , block_num);
    int page_id = BM.getPageId(TABLE_MANAGER_PATH , block_num);
    strcat(buffer , str_tmp.c_str());
    BM.modifyPage(page_id);
...
void CatalogManager::dropTable(std::string name){
    ...
    //得到所对应块的信息
    char* buffer = BM.getPage(TABLE_MANAGER_PATH , suitable_block);
    int page_id = BM.getPageId(TABLE_MANAGER_PATH , suitable_block);
